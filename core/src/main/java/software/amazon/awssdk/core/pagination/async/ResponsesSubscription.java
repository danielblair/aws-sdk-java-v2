...
public class ResponsesSubscription<T> implements Subscription {
    private final Subscriber subscriber;

    private AtomicLong outstandingRequests = new AtomicLong(0);

    private AsyncPageFetcher<T> nextPageFetcher;

    private volatile T currentPage;

    private AtomicBoolean isTaskRunning = new AtomicBoolean(false);

    public ResponsesSubscription(Subscriber subscriber, AsyncPageFetcher<T> nextPageFetcher) {
    ...
    private synchronized void handleRequest() {
        ...
        CompletableFuture<T> future = nextPageFetcher.nextPage(currentPage);
